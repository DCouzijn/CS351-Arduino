...
int totalScore = 0;
int swapCounter = 0;
//Serve & receiver condition
boolean ServeSwap = false;
//server count
int ServeCounter = 1;
//table swap condition
boolean SideSwap = false;
//win condition (has someone won the game yet)
boolean win = false;
//Set Counter (best of five sets or first to 3 sets)
int P1Set = 0;
int P2Set = 0;
...
void loop() {
  ...
  //if players are swapped from their original starting positions swap button functions
  if(SideSwap == true){
  ...
  //display server as LED lit up as long as a player has not won the overall game
  if(win == false){
    if(ServeSwap == false){
      delay(200);
  ...
  totalScore = P1Score + P2Score;
    //player set score must be >=11 and difference between two players must be >=2 for a player to win a set
    if(P1Score >= 11 && (P1Score - P2Score) >= 2){
      ...
      if(SideSwap == false){   //swaps the sides of players if not swapped from original positions
        SideSwap = true; 
        lcd.setCursor(0,0);
        lcd.print("P2");
        lcd.setCursor(14,0);
      ...
      } else{  //swaps sides of players to original sides if swapped 
        SideSwap = false;
         lcd.setCursor(0,0);
        lcd.print("P1");
        lcd.setCursor(14,0);
